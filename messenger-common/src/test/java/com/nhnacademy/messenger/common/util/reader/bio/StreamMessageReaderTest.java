...
class StreamMessageReaderTest {
    ...
    void read_Message_HeaderTooLong() {
        // Given: \n 없이 101글자의 더미 헤더
        ...
        // When & Then
        assertThrows(MessageConvertException.class, reader::readMessage, "헤더 길이가 제한을 초과했습니다.");
    }

    @Test
    @DisplayName("예외: 헤더 형식이 'message-length: '로 시작하지 않으면 MessageConvertException 발생")
    void read_Message_InvalidHeaderPrefix() {
        // Given
        ...
        // When & Then
        assertThrows(MessageConvertException.class, reader::readMessage, "유효하지 않은 프로토콜 헤더입니다");
    }

    @Test
    @DisplayName("예외: 헤더의 길이가 숫자가 아니면 MessageConvertException 발생")
    void read_Message_InvalidHeaderLength() {
        // Given
        ...
        // When & Then
        assertThrows(MessageConvertException.class, reader::readMessage, "메시지 길이를 파싱할 수 없습니다");
    }

    @Test
    @DisplayName("예외: 읽을 데이터가 없는 상태(EOF)에서 read 호출 시 EOFException 발생")
    void read_Message_EOF_BeforeHeader() {
        // Given: 빈 스트림
    ...
    void read_Message_EOF_DuringHeader() {
        // Given: \n 없이 끊김
        ...
        // When & Then
        assertThrows(EOFException.class, reader::readMessage, "헤더를 읽는 도중 연결이 종료되었습니다.");
    }

    @Test
    @DisplayName("예외: 바디를 읽는 도중(길이 부족)에 스트림이 끊기면 EOFException 발생")
    void read_Message_EOF_DuringBody() {
        ...
        // When & Then
        assertThrows(EOFException.class, reader::readMessage, "메시지 본문을 읽는 도중 연결이 종료되었습니다.");
    }

    @Test
    @DisplayName("예외: 헤더와 길이는 맞지만 JSON 형식이 잘못되었을 때 MessageConvertException 발생")
    void read_Message_InvalidJson() {
        // Given
        ...
        // When & Then
        assertThrows(MessageConvertException.class, reader::readMessage, "역직렬화 실패");
    }
}